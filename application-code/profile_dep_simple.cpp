...
arma::vec profile_likelihood(arma::vec beta, arma::vec lambda_initial, arma::mat X_dep, 
  ...
  while (error > eps && Iter < MaxIter){
    ...
    Iter = Iter + 1;
    std::cout << "Lambda Iter = " << Iter << ", Error = " << error << std::endl;
  }
...
// [[Rcpp::export]]
Rcpp::List get_W(arma::vec beta_new, arma::mat X_dep_new, arma::vec lambda_old,
                 arma::mat simpdat, double eps, arma::vec Time, double constant, 
                 arma::mat Dpl_initial){
  Rcpp::List ret;
  long n = simpdat.n_rows;
  ...
  Dpl.submat(0,0,n-1,p-2) = Dpl_initial;
  
  //first order
  for(long j = p-1; j < p; ++j){
    ...
    for(long i = 0; i < n; ++i){
      Dpl(i,j) = (likelihood_i(beta_j1, lambda_j1, X_dep_new, simpdat, Time, i) - 
        likelihood_i(beta_j2, lambda_j2, X_dep_new, simpdat, Time, i)) / (2*h);
    }
  }
