...
library(tidyverse)
library(latex2exp)
load("./output/Figure2/ff_twisted_n200_raw.RData")

p_ff_cates_n200 = ggplot(data = df, aes(x = r, y = power, color = Test, shape = Test)) +
  geom_point(size = 2) +
  geom_line() + 
  labs(title = "Twisted functional form, n = 200") +
