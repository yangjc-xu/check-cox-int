...
library(tidyverse)
library(latex2exp)
load("./output/Figure2/ff_twisted_n400_raw.RData")

p_ff_cates_n400 = ggplot(data = df, aes(x = r, y = power, color = Test, shape = Test)) +
  geom_point(size = 2) +
  geom_line() + 
  labs(title = "Twisted functional form, n = 400") +
...
save(p_ff_cates_n400, file = "./output/Figure2/ff_twisted_n400_plot.RData")
