...
library(tidyverse)
library(latex2exp)
load("./output/Figure2/ff_quad_n800_raw.RData")

p_ff_optimal_n800 = ggplot(data = df, aes(x = r, y = power, color = Test, shape = Test)) +
  geom_point(size = 2) +
  geom_line() + 
  labs(title = "Additional quadratic term, n = 800") +
