...
library(tidyverse)
library(latex2exp)
load("./output/Figure1/cubic_raw.RData")
df = data.frame(y = colMeans(W_score_all), x = x)
intervals <- list(c(0, 3))
smooth_piecewise <- function(data, intervals, span = 0.2) {
...
p_cubic = ggplot() +
  ...
  labs(x = "Covariate", y = "Cumulative statistic", title = "(f)")+
  scale_x_continuous(limits = c(0,3), breaks = seq(0,3,0.5), expand = expansion(0,0)) +
  scale_y_continuous(limits = c(-0.9,0.6), breaks = c(seq(-0.9,-0.3,0.3), seq(0,0.6,0.3)), expand = expansion(0,0))+
