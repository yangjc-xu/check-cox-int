...
library(dplyr)
library(Rcpp)
sourceCpp("./simulation-code/prop_n400_quad/unireg_indep.cpp")
sourceCpp("./simulation-code/prop_n400_quad/profile_dep_fast.cpp")
##################simulation##############################################
...
p_prop_quad_n400 = ggplot(data = df) +
  theme_light() +
  ...
  labs(x = "Time", y = "Cumulative statistic", title = "Quadratic log hazard ratio, n = 400") +
  scale_x_continuous(limits = c(0,5), breaks = seq(0,5,0.5), expand = expansion(0,0)) +
  scale_y_continuous(limits = c(-2.5,2.5), breaks = c(seq(-2.5,0,0.5), seq(0.5,2.5,0.5)), expand = expansion(0,0)) +
