...
library(tidyverse)
library(latex2exp)
load("./output/Figure3/prop_mono_n800_raw.RData")

p_prop_optimal_n800 = ggplot(data = df, aes(x = r, y = power, color = Test, shape = Test)) +
  geom_point(size = 2) +
  geom_line() + 
  geom_line(data = df, aes(x=r, y=power)) +
  labs(title = "Monotone hazard ratio, n = 800") +
