...
library(tidyverse)
library(latex2exp)
load("./output/Figure3/prop_quad_n800_raw.RData")

p_prop_quad_n800 = ggplot(data = df, aes(x = r, y = power, color = Test, shape = Test)) +
  geom_point(size = 2) +
  geom_line() + 
  labs(title = "Quadratic log hazard ratio, n = 800") +
