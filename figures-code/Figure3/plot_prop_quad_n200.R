rm(list = ls())
library(tidyverse)
load("./output/Figure3/prop_quad_n200_raw.RData")

p_prop_quad_n200 = ggplot(data = df, aes(x = r, y = power, color = Test, shape = Test)) +
  geom_point(size = 2) +
  geom_line() + 
  labs(title = "Quadratic log hazard ratio, n = 200") +
...
save(p_prop_quad_n200, file = "./output/Figure3/prop_quad_n200_plot.RData")
