...
library(dplyr)
library(Rcpp)
sourceCpp("./simulation-code/ff_n400_twisted/unireg_indep.cpp")
sourceCpp("./simulation-code/ff_n400_twisted/profile_new.cpp")
##################simulation##############################################
...
for (l in 1:length(r_seq)) {
  ...
  for (k in 1:n_simulation) {
    ...
    ## Score test
    X_score = SimData$X2^2
    X_new_score = cbind(EstRes$X, X_score)
    res_score = get_W(beta_new, X_new_score, EstRes$lambda, EstRes$simpdat, 0.001, EstRes$time, constant)
    Stat = res_score$W/sqrt(n)
    A = e - res_score$Info[p+1,1:p] %*% solve(EstRes$I_first[1:p,1:p]) %*% H
    Var = A %*% res_score$Info %*% t(A)
    Test = Stat/sqrt(Var)
    p_score[k] = 2*(1-pnorm(abs(Test)))
    
    ## Score test
    X_score_cube = SimData$X2^3
    X_new_score_cube = cbind(EstRes$X, X_score_cube)
    res_score_cube = get_W(beta_new, X_new_score_cube, EstRes$lambda, EstRes$simpdat, 0.001, EstRes$time, constant)
    Stat_cube = res_score_cube$W/sqrt(n)
    A_cube = e - res_score_cube$Info[p+1,1:p] %*% solve(EstRes$I_first[1:p,1:p]) %*% H
    Var_cube = A_cube %*% res_score_cube$Info %*% t(A_cube)
    Test_cube = Stat_cube/sqrt(Var_cube)
    p_score_cube[k] = 2*(1-pnorm(abs(Test_cube)))
  }
  
  power1[l] = mean(p1 < 0.05)
  power_score[l] = mean(p_score < 0.05)
  power_score_cube[l] = mean(p_score_cube < 0.05)
}
###################################################################################
