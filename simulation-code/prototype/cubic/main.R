...
library(dplyr)
library(Rcpp)
sourceCpp("./simulation-code/prototype/cubic/unireg_indep.cpp")
sourceCpp("./simulation-code/prototype/cubic/profile_new.cpp")
##################simulation##############################################
...
SimIntData = function(n_sample, r){
  ...
  linear_term =  - r*(X2-0.5)*(X2-1.5)*(X2-2.5)
...
get_est = function(SimData){
  ...
  X_simulated = as.matrix(model.matrix(~ X2 + I(X2^2), SimData)[,-1])
  ## Model fitting
  startT = Sys.time()
...
beta_est = matrix(0, nrow = n_simulation, ncol = 2)
