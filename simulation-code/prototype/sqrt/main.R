...
library(dplyr)
library(Rcpp)
sourceCpp("./simulation-code/prototype/sqrt/unireg_indep.cpp")
sourceCpp("./simulation-code/prototype/sqrt/profile_new.cpp")
##################simulation##############################################
...
SimIntData = function(n_sample, r){
  ...
  X2 = runif(n = N, 0, 3)
  linear_term = r * sqrt(X2)
  
  
  EventTime = rep(NA, N)
  for (i in 1:N) {
    Time = rexp(n = 1, rate = exp(linear_term[i]))
    EventTime[i] = 10*sqrt(Time)
  }
